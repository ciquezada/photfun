...
C
     ...
     .     MAXSKY, MAXPAR
      PARAMETER (MAXCOL=16384, MAXROW=16384)
      PARAMETER (NOPT=20, NCMD=23, MAXPSF=407)
      PARAMETER (MAXEXP=10, MAXPAR=6, MAXBOX=57, MAXSKY=16384)
C
C Parameters
C
...
C
C MAXSKY must be >= MCOL  is the maximum number of pixels that will
C        be used in any sky determination.  This will affect the speed of the
...
C
      ...
      CHARACTER ANSWER*1
      REAL SKYMN, SKYMED, SKYMOD, SKYSIG
...
C
     ...
     .            0., -3., 1., .5, -1.5, -0.5, -7.49, 0., 0., 0., 0./
      DATA OMAX / 1.E30, 1.E30, 1.E30, 9999999., 39., 1.E30, 1.0,
     .            2., 0., 2., 3., 49., 101., 3.5, 3.5, 7.49, 9.5, 1.,
     .            100., 100./
C
...
C
      ELSE IF (KEY .EQ. ICNVRT('OP')) THEN
         CALL TBLANK
         OPTFIL = 'KEYBOARD INPUT'
         CALL GETNAM ('File with parameters:', OPTFIL)
...
C
      ELSE IF (KEY .EQ. ICNVRT('SK')) THEN
         IF (IMID(1) .GT. 0) THEN
            MAX = MAX0(MAXCOL, MIN0(MAXSKY, MAXSIZ/3))
            CALL GETSKY (WORK, WORK(MAX+1), INDEX, MAX, 
     .           OPT(1), OPT(4), SKYMN, SKYMED, SKYMOD, SKYSIG, K)
         ELSE
            CALL STUPID ('   No picture file has been ATTACHed.')
         END IF
C
...
C
      ELSE IF (KEY .EQ. ICNVRT('DU')) THEN
         IF (IMID(1) .GT. 0) THEN
            CALL DUMP (WORK, NCOL, NROW, OPT(4), INDEX)
         ELSE
            CALL STUPID ('   No picture file has been ATTACHed.')
         END IF
C
...
C
      IMPLICIT NONE
      CHARACTER LINE*400, IFILE1*40, IFILE2*40, SWITCH*40, CASE*4
...
C#######################################################################
C
      SUBROUTINE  DUMP  (F, NCOL, NROW, HIBAD, INDEX)
C
C=======================================================================
C
...
C
      ...
      DO 1020 J=NY,1,-1
      WRITE (6,611) LY+J-1, 
     .     (NINT(AMAX1(-99999.,AMIN1(999999.,F(I,J)))), I=1,NX)
  611 FORMAT (I6, ' |', 21I7)
      DO 1010 I=1,NX
      IF (F(I,J) .LE. HIBAD) THEN
         N=N+1
         D(N)=F(I,J)
      END IF
 1010 CONTINUE
...
c        c = c+d(i)
c     end do
c     print*,(d(i),i=n,n-4,-1),c
C
      IF (NBOX .LT. NSQUARE) CALL TBLANK
      WRITE (6,612) NINT(AMAX1(-99999.,AMIN1(999999.,D(1)))), 
     .     NINT(AMAX1(-99999.,AMIN1(999999.,
     .     0.5*D((N+1)/2)+0.5*D((N/2)+1)))), 
     .     NINT(AMAX1(-99999.,AMIN1(999999.,D(N))))
...
C
 ...
 3000 I = I+1
      ...
      IF (NLINE1 .GE. I) THEN
         WRITE (3,301) ID, STRNG1, STRNG2, AMAG, LINE1(I:NLINE1)
  301    FORMAT (I7, 2A9, F9.3, A)
      ELSE
         WRITE (3,301) ID, STRNG1, STRNG2, AMAG
      END IF
