...
C
      PARAMETER (MAXAP=12, MAXSTR=1000, MAXFIL=20000, MTERM=5, NOPT=14)
...
C
 1100 READ (2,*,END=2000) ID(NSTAR), X(NSTAR), Y(NSTAR), 
     .     (MAG(I,NSTAR), I=1,NDIFF)
      ...
      READ (LINE,*,iostat=istat) SKY(NSTAR), S, SKEW, 
     .     (SIG(I,NSTAR), I=1,NDIFF)
      IF (ISTAT .NE. 0) THEN
         ISTAT = 0
         READ (LINE,601,IOSTAT=ISTAT) SKY(NSTAR), S, SKEW,
     .     (SIG(I,NSTAR), I=1,NDIFF)
  601    FORMAT (F14.3, 2F6.3, F8.4, 11F9.4)
         if (istat .ne. 0) then
            print*,'"'//line(1:13)//'"'//line(26:33)//'"'//
     .        line(34:42)//'"'
            print*,file(nfile),id(nstar)
            istat = 0
         end if
      END IF
      if (istat.ne.0) then
...
C
      DO I=1,NSTAR
         ...
         END IF
      END DO
      WRITE (LINE,602) 'A', 'B', 'C', 'D', 'E', 'sigma'
  602 FORMAT (1X, 5(9X, A1, 3X), 7X, A5)
...
C
  ...
  301    FORMAT (2X, '-->', I5.4, 10X, A8, 12F9.4)
         ...
         WRITE (3,301) K, ' Sigma_O', (WOBS(J), J=2,MAP)
         WRITE (3,301) K
         WRITE (3,301) K, ' Adopted', (ADOPT(J), J=2,NDIFF)
         WRITE (3,301) K, ' Sigma_A', (WADO(J), J=2,NDIFF)
         WRITE (3,301) K
         L = NDIFF+1
         IF (HST) THEN
            CUM(L) = 0.
  ...
  303    FORMAT (2X, '-->', I5.4, 4X, A10, 12F9.4)
         WRITE (3,303) K, '   Sigma_C', (SQRT(WCUM(J)), J=2,NDIFF)
         WRITE (3,*)
         WRITE (3,*)
