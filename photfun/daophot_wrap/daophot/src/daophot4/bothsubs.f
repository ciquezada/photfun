...
C
      ...
      if (imgtyp(jmno) .eq. 'imh') then
         ...
         call ftgiou (j, ier)
c     print*,'delpic ftgiou', ier
         call delfil (j, name, ier)
c     print*,'delpic delfil', ier
c     print*,message(imgtyp(jmno), ier)
         ier = 0
         call ftfiou (j, ier)
c     print*,'delpic ftfiou', ier
      end if
      if (ier .ne. 0) then
...
C
      if(show)print*,'opnpic ',pictur(imno),mode,imid(imno)
      ier = 0
C
...
C
      ...
      if(show)print*,'mode ',j,'  imagetype="'//imgtyp(imno)//'"'
C
...
c     print*,pictur(1)
c     print*,pictur(2)
c     print*,ncol(2),nrow(2),imgtyp(2),dattyp(2)
      call crepic (1, 2, ier)
...
c        IF (ACCESS(NAME, 'W') .EQ. 0)      PRINT*,'NO DELETE',IER
      end if
      ier = 0
      ncol(jmno) = axlen(1)
      nrow(jmno) = axlen(2)
...
c     PRINT*,NAME
     ...
     .           ' to '//pictur(jmno))
         ...
         ier = 0
c     PRINT*,'BEFORE FTCOPY', imno, imid(imno), j
