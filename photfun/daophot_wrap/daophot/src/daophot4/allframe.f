...
C                              allframe.f
C
C                 Official DAO version: 2016 September 12
C
C#######################################################################
C
C
...
C
      ...
      PARAMETER (MAXSTR=1 000 000, MAXLST=40 000 000)
      PARAMETER (MAXCOL=12 000, MAXROW=12 000, RAT=1)
C
...
C
      ...
      PARAMETER (NOPT=10, MAXPSF=407, MAXPAR=6, MAXEXP=10)
...
C
     ...
     .     HIBAD(MAXPIC/3), PHPADU(MAXPIC/3), RONOIS(MAXPIC/3)
      REAL SKYOUT(MAXPIC/3), SKYISQ(MAXPIC/3), SKYOSQ(MAXPIC/3)
      REAL PAR(MAXPAR)
...
C
      ...
      REAL PERERR, PROERR, DUM, PSFMAG, BRIGHT, SIGLIM,
     ...
     .     WT, DWT, RANGE, SNMIN, SEPMAX, SEP1, XMIN, XMAX, YMIN, YMAX
      ...
      REAL RATIO, SOFT, SHPMIN, SHPMAX, SHPCRT, RMINSQ, RMAXSQ
...
C
     ...
     .         '     OS (OUTER SKY RADIUS)'/     ! 10
      DATA OPT / 6., 2.5, 6., 2., 5., 200., 0.75, 1., 1., 15./
      DATA OMIN  / 0., 0., -0.5, -3.5, 0., 1., 0., 0., 0., 0.5/
      DATA OMAX  / 16., 100., 20.5, 3.5, 100., 1000., 100., 100.,
     .     30., 120./
      DATA CENTER /.TRUE./, CRDMAX /400./, NDELTA / 0 /
      data imid /maxpic*0/
      data soft /2./, ratio /1.4/
      DATA SHPMIN /-20./, SHPMAX/40./, SIGLIM/0.2995/
C     DATA SHPMIN /-20./, SHPMAX/40./, SIGLIM/0.3995/
C
...
C
     ...
     .        PROERR, NADJ, CLIP, SEPMAX, CRDMAX, START, NDELTA
C
         J = 3*NPIC
         READ (1,END=900,ERR=900) (PICTUR(I), MODE(I), 
     .        IMGTYP(I), DATTYP(I), I=1,J)
C
         READ (1,END=900,ERR=900) (NCOL(I), NROW(I), XCEN(I), 
     .        XWID(I), YCEN(I), YWID(I), FITRAD(I), LOBAD(I), 
     .        HIBAD(I), RONOIS(I), PHPADU(I), DMAG(I), NOUT(I), 
     .        SEPCRT(I), SKYOUT(I), SKYISQ(I), SKYOSQ(I),
...
C
     ...
     .           (COEFF(I,NPIC), I=1,6), DMAG(NPIC)
      ...
      END IF
C
      DUM = ABS(COEFF(3,NPIC)*COEFF(6,NPIC)) +
     .      ABS(COEFF(4,NPIC)*COEFF(5,NPIC))
      SKYISQ(NPIC) = OPT(9)**2/DUM
      SKYOSQ(NPIC) = OPT(10)**2/DUM
      SKYOUT(NPIC) = SQRT(SKYOSQ(NPIC))
C
      NL = LENGTH(PICTUR(NPIC))
C
C Ignore any .stc, .2ma, .nmg, .mag, .fet, .dss, .pds, or .in file.
C
      IF (PICTUR(NPIC)(NL-3:NL) .EQ. CASE('.STC')) GO TO 1915
      IF (PICTUR(NPIC)(NL-3:NL) .EQ. CASE('.stc')) GO TO 1915
      IF (PICTUR(NPIC)(NL-3:NL) .EQ. CASE('.2ma')) GO TO 1915
      ...
      IF (PICTUR(NPIC)(NL-3:NL) .EQ. CASE('.fnl')) GO TO 1915
      IF (PICTUR(NPIC)(NL-2:NL) .EQ. CASE('.in')) GO TO 1915
C
...
C
     ...
     .     FFEOC(1,NPIC))
      call back (-48.986,41.813,
     .           xcen(npic), xwid(npic), ycen(npic), ywid(npic),
     .           ffeoc(1,npic), mode(npic), dx, dy)
      GO TO 1910
C
 1920 NPIC = NPIC-1
      CALL OVRWRT ('                                        ', 2)
...
C
     ...
     .        MAXEXP, NPSF, NEXP, NFRAC, PSFMAG, BRIGHT, XPSF, YPSF)
         ...
         END IF
         IF (RAT .GT. 0.) THEN
              NUMER = SQRT(SOFT**2+(RATIO*(PAR(1)+PAR(2)))**2)
              FITRAD(IMNO) = MIN(49., NUMER)
         END IF
         IF ((FITRAD(IMNO) .LT. 1.) .OR. (FITRAD(IMNO) .GT. 49.)) THEN
...
C
     ...
     .           (DY .LT. XMIN) .OR. (DY .GT. YMAX)) THEN
      ...
      IF (NLIST .GT. MAXLST) THEN
         ...
         INPFIL = COMMAS(MAXLST, J)
         WRITE (6,*) OPTFIL(1:I)//'   '//INPFIL(1:J)
         CALL OOPS
      END IF
      NCNVRG = 0
      NDISAP = 0
...
C
C Stars ISTAR and JSTAR are too close together.  Combine their
C properties.  This means average their positions; sum their
C fluxes and average the flux errors, chi, and sharp in images
C where they both appear.  Store these as ISTAR and discard JSTAR.
C
     ...
     .                  MAGERR(LSTAR))
               ...
               ELSE
C
C Star ISTAR doesn't appear in this image, but star JSTAR does, so ISTAR
C inherits JSTAR's properties.
C
                  IWHICH(ISTAR,IMNO) = IWHICH(JSTAR,IMNO)
               END IF
            END DO
            DXOLD(ISTAR) = 0.
            DYOLD(ISTAR) = 0.
...
C
      ...
      IF (DOSKY) THEN
         RADIUS = AMAX1(SRADSQ, SKYOUT(IMNO))
      ELSE
         RADIUS = SRADSQ
      END IF
      SRADSQ = SRADSQ**2
...
C
               ...
               GO TO 2198
            END IF
            LX = MAX0(IXMIN, INT(X(ISTAR)-RMAX)+1)
            MX = MIN0(IXMAX, INT(X(ISTAR)+RMAX))
            LY = MAX0(JYMIN, INT(Y(ISTAR)-RMAX)+1)
            MY = MIN0(JYMAX, INT(Y(ISTAR)+RMAX))
C
            N = 0
            DO 2195 JY=LY,MY
               DYSQ = (REAL(JY) - Y(ISTAR))**2
...
c        IF (NITER .GT. ITCONV) THEN
c           IF (SHP(KSTAR) .LT. SHPMIN*SHPCRT) THEN
c              iwhich(istar,imno) = -7
c              dxwt(istar) = -7.
c              GO TO 2299
c           ELSE IF (SHP(KSTAR) .GT. SHPMAX*SHPCRT) THEN
c              iwhich(istar,imno) = -7
c              dxwt(istar) = -7.
c              GO TO 2299
c           END IF
...
C
            XMAX = ABS(SNGL(ZX(1)))
            IF (XMAX .GT. 0.1*AMAG(KSTAR)) THEN
               DONE(ISTAR) = .FALSE.
            ELSE IF (XMAX**2 .GT. 0.04*MAGERR(KSTAR)) THEN
               DONE(ISTAR) = .FALSE.
            END IF
         END IF
C
...
C
 ...
 4500 CONTINUE
      MX = NTFM
C
C If there is only one star, update the positions in the
C individual images, but do NOT recompute the transformations.
C
      IF (L .LE. 1) GO TO 4900
 ...
 4900 CONTINUE
      DO ISTAR=1,NSTAR
         IF (DXWT(ISTAR) .GT. 0.) THEN
            ...
            DWT = 1./DWT
            DX = (COEFF(3,IMNO)*DXC(ISTAR)+COEFF(5,IMNO)*DYC(ISTAR))
C
C EXPERIMENT!!!
C Whatever the scale of this particular image, don't allow it to
C request a positional shift of more than XCLAMP,YCLAMP pixels.
C
            DX = DX/(1.+ABS(DX)/XCLAMP(ISTAR))                          !!!
            XSUM(ISTAR) = XSUM(ISTAR) + DWT*DX
            XWT(ISTAR) = XWT(ISTAR) + DWT
            DWT = CFFSQ(4,IMNO)/DXWT(ISTAR) + CFFSQ(6,IMNO)/DYWT(ISTAR)
            DWT = 1./DWT
            DY = (COEFF(4,IMNO)*DXC(ISTAR)+COEFF(6,IMNO)*DYC(ISTAR))
            DY = DY/(1.+ABS(DY)/YCLAMP(ISTAR))                          !!!
            YSUM(ISTAR) = YSUM(ISTAR) + DWT*DY
...
C
     ...
     .        ID(NSTAR), ' disappear signal-to-noise 1 NDISAP =', ndisap
         GO TO 5003
      ELSE IF (DWT .LT. 9.) THEN
         IF (ABS(WATCH) .GT. 2.5) PRINT *,
     .        ID(NSTAR), ' continue signal-to-noise 1'
         DONE(NSTAR) = .FALSE.
      END IF
C
C Now check the rest of the stars one by one.
C
 5005 ISTAR = ISTAR+1
C
      IF ((WATCH .GT. 0.5) .AND. 
...
C
      ...
      END IF
C
C If this star is bad, replace it with the last star.
C
      IF (DWT .LT. SNMIN) THEN
         IF (ABS(WATCH) .GT. 2.5) PRINT *, ID(ISTAR), 
     .        '  disappear signal-to-noise 2', DWT, SNMIN, 
     .        '   NDISAP =', ndisap+1
         ID(ISTAR) = ID(NSTAR)
         XC(ISTAR) = XC(NSTAR)
         YC(ISTAR) = YC(NSTAR)
         ...
         NSTAR = NSTAR-1
C
C This star was bad and was replaced with the last star.  Now check
C to see whether the new last star is good or bad.
C
         GO TO 5003
      ELSE IF (DWT .LT. 9.) THEN
         DONE(ISTAR) = .FALSE.
         IF (ABS(WATCH) .GT. 2.5) PRINT *,
     .        ID(ISTAR), '  continue signal-to-noise 2'
      END IF
C
C This star was not bad and was not replaced.  Check the next star.
C
      GO TO 5005
C
 5007 CONVRG = .FALSE.
      DO ISTAR=1,NSTAR
         ...
         END IF
         DXOLD(ISTAR) = DX
CXYZ     DX = DX/(1.+ABS(DX/XCLAMP(ISTAR)))
         XC(ISTAR) = XC(ISTAR)+DX
         IF (DONE(ISTAR)) THEN
            IF (DX**2 .GT. AMAX1(0.01/XWT(ISTAR), 1.E-6) ) THEN
               DONE(ISTAR) = .FALSE.
               IF (ABS(WATCH) .GT. 2.5) PRINT *, id(istar),
     .              '  continue DX =', DX
            END IF
...
C
 5008    IF (YWT(ISTAR) .LE. 0.) GO TO 5009
         ...
         END IF
         DYOLD(ISTAR) = DY
CXYZ     DY = DY/(1.+ABS(DY/YCLAMP(ISTAR)))
         YC(ISTAR) = YC(ISTAR)+DY
         IF (DONE(ISTAR)) THEN
            IF (DY**2 .GT. AMAX1(0.01/YWT(ISTAR), 1.E-6) ) THEN
               DONE(ISTAR) = .FALSE.
               IF (ABS(WATCH) .GT. 2.5) PRINT *, id(istar),
     .              '  continue DY =', DY
            END IF
...
C
 5009    CONTINUE
      ...
      END DO
cC
cC Only subtract stars if a sufficient fraction, taken to be at least 2%,
cC have converged.
C
c     IF (NDONE .LT. NINT(0.02*NSTAR)-NDELTA) THEN
c        CONVRG = .FALSE.
c        NDELTA = NDELTA+1
c     ELSE
c        NDELTA = 0
...
C
      IF (START) THEN
         ...
         DX = AMAX1(-XMIN, XMAX)
         DY = AMAX1(-YMIN, YMAX)
         CALL WRHEAD (1, 1, NINT(DX), NINT(DY), 
...
C
C Check sigma and sharp for convergence
C
      DPOS = 1.085736*SQRT(MAGERR(KSTAR))/AMAG(KSTAR)
      IF (DPOS .GE. SIGLIM) THEN
         MAGERR(KSTAR) = -10.
         IWHICH(ISTAR,IMNO) = 0
         GO TO 6004
      END IF
C
c     IF ((SHP(KSTAR) .LT. SHPMIN) .OR. (SHP(KSTAR) .GT. SHPMAX)) THEN
c        IWHICH(ISTAR,IMNO) = -8
c        DONE(ISTAR) = .FALSE.
c        GO TO 6004
c     END IF
C
...
C
      IF ((JYMAX-JYMIN+1) .LE. 10.*NDONE*NPSF) THEN
C
...
C
     ...
     .      FFEOC)
C
C The footprint of THIS IMAGE in the MASTER COORDINATE SYSTEM is
C mapped to [-1:1,-1:1], more or less, and polynomial approximations
C to the inverse transform are obtained.
C
