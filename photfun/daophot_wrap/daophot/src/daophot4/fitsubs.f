...
c     if (show) print*,'y',imno, imid(imno), ifirst, n, ier
         call ftgpve (imid(imno), 1, ifirst, n, 1.01e19, row, 
     .        bad, ier)
c     if (show) print*,'Y ',bad,ier
         if (ier .ne. 0) then
            write (6,*) ' ftgpve in rdrow'//pictur(imno),
     .           imno,imid(imno),ncol(imno), iy, ier
            call stupid ('rdrow: '//message(imgtyp(imno), ier))
   99       call FTGMSG (text)
            if (text .eq. ' ') return
            print*,text
...
c
      ...
      else if ((imgtyp(imno) .eq. 'fits') .or.
     .         (imgtyp(imno) .eq. 'fits')) then
         i = (ly-1)*nx+1
         n = (my-ly+1)*nx
         bad = .false.
