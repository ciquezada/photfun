...
C
      include 'arrays.inc'
C
      PARAMETER (MAXOPN=101, MAXUSE=101, MINSKY=1000)
      PARAMETER (MXINC=16384, MXOUTC=16384, MXOUTR=16384)
C
     ...
     .     EXTEND*40, SQUEEZ*40, TEMP*40
      ...
      CHARACTER LIMITS*32, STRNG1*19, STRNG2*19, STRNG3*19
     ...
     .     LOBAD(MAXPIC)
      ...
      LOGICAL ALIGN(MAXPIC), ROTATE, USEPDS, USEMAG, SHOW, USED
      LOGICAL AUTO
C
...
C
  ...
  705 I = I-1
     ...
     .     (NAME(I:I) .EQ. '!') .OR. (NAME(I:I) .EQ. '&')) THEN
         IF (NAME(I:I) .EQ. '/') THEN
...
C
  ...
  755 CALL RDCHAR (1, LINE, N, IER)
     ...
     .           (COEFF(J,NFRAME), J=1,6), DMAG(NFRAME)
      ...
      END IF
C
C Ignore all .stc, .2ma, .mag, .nmg, .fet, .jnk, .in and .dss files.
C
      do i=1,27
         ...
         if (file(nframe)(i:i+3) .eq. '.in ') go to 755
...
C
         ...
         WRITE (6,697) XLO(IMNO), XHI(IMNO), YLO(IMNO), YHI(IMNO), 
...
C
         XX = XX + YY
C
C Add 1.5 pixels in quadrature to prevent a severely undersampled image
C from dominating.
C
         XX = SQRT(XX**2 + 2.25)
C
...
c        WEIGHT(IMNO) = 1./DMAG(IMNO)**2
C
C Give greater weight to better-seeing frames.
C
         WEIGHT(IMNO) = WEIGHT(IMNO)/HOLD(IMNO)
c        WEIGHT(IMNO) = WEIGHT(IMNO)/HOLD(IMNO)**2 / dy    !xyz
...
C
 ...
 1901 IF (AUTO) THEN
         ...
         IEXPND = 1
      ELSE
         CALL GETDAT ('X limits of output image:', DATUM, 2)
         ...
         IEXPND = NINT(DATUM(1))
         IEXPND = MIN0( IEXPND, 16384/(IXMAX-IXMIN+1),
     .        16384/(IYMAX-IYMIN+1) )
...
C
      ...
      WRITE (LIMITS,2) IXMIN, IXMAX, IYMIN, IYMAX
    2 FORMAT (4I8)
...
C
      ...
      CALL CLPIC (1, IER)
      IF (USED) THEN
         temp = switch(name, '.use')
         call delfil (9, temp, istat)
         call outfil (9, temp, istat)
      END IF
C
      DO I=1,30
         IF (NAME(I:I) .EQ. ':') THEN
 ...
 1990 I = LENGTH(NAME)
  ...
  198       FORMAT ('Montage of ', A, ' images, scale = ', A, 
     .           ' px/arcsec')
      ...
      END IF
      CALL SPUT (MAXPIC, 'LIMITS', LIMITS, ' ', IER)
...
C
 6498       DD = ROW(I,IMNO)
c     print*,imno,dd,lobad(imno),hibad(imno)
            IF ((DD .GE. LOBAD(IMNO)) .AND.
     .           (DD .LE. HIBAD(IMNO))) THEN
               M = M+1
               DATUM(M)=(DD-SKY(IMNO))*DMAG(IMNO)
...
c     print*,'A',m,nmax
     ...
     .                     (X-HOLD(J))/(HOLD(I)-HOLD(J))
          ...
          END IF
 6900   CONTINUE
c       print*,work(jcol,jrow), jcol,jrow
c       read (5,*)
 6950   CONTINUE
 6975   CONTINUE
...
C
 ...
 3100 DO IMNO=1,NFRAME
         ...
         IF (USED) THEN
            IF (NUSED(IMNO) .GT. 0) WRITE (9,8) NUSED(IMNO),
     .           WEIGHT(IMNO),
     .           PICTUR(IMNO)(1:LENGTH(PICTUR(IMNO)))//' '
    8       FORMAT (I14, F9.3, 2X, A)
         END IF
      END DO
      CALL OVRWRT ('                           ', 3)
      IF (USED) THEN
         CALL CLFILE (9)
         WRITE (6,*) NTOT, ' pixels defined, ==>',
     .        REAL(NTOT)/(60.*REAL(IEXPND))**2, ' square arcminutes'
         CALL TBLANK
      END IF
      IF (AUTO) WRITE (6,*) ' '//NAME
      CALL BYEBYE
      END
C
C#############################################################################
...
C3666 TYPE *,' STACK: ',STKLO(I),STKHI(I)
      GO TO 100
C
  300 CONTINUE
C
...
C4666 TYPE *,' STACK: ',STKLO(I),STKHI(I)
      GO TO 100
C
  400 CONTINUE
C
