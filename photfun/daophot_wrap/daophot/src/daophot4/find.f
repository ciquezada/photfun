...
c     HMIN = AMAX1(HMIN, SUM2)
      DY = (OPT(3)-1.)*HMIN + SUM2
      LOBAD = 0.1*NINT(10.*(SKYMOD-DY))
c     LOBAD = 0.1*NINT(10.*(SKYMOD-OPT(3)*DY))
...
C
      ...
      DY = 2.*(HX-HY)/(HX+HY)
cC
cC Do not apply this roundness criterion if an off-central pixel is saturated.
cC
c      IF (.NOT. SATUR8) THEN
