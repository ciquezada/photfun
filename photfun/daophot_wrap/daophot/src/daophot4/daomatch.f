...
C=======================================================================
C
      PARAMETER (MAXSTR=40, MAXTRI=9880, NRRES=30, NZRES=30)
...
C
      ...
      LOGICAL LIMIT, SKIP, SETSCA, SETROT, AUTO
C
...
C
         CALL TBLANK
         CALL GETNAM ('Master input file:', FILE)
...
C
 ...
 1000 IF (AUTO) THEN
         CALL GETARG (2, FILE)
         IF (FILE .EQ. ' ') AUTO = .FALSE.
      END IF
      IF (.NOT. AUTO) CALL GETNAM ('Next input file:', FILE)
...
C
      IF (LIMIT) THEN
         CALL GETDAT ('Xmin, Xmax, Ymin, Ymax:', DATA, 4)
         ...
         YMIN = AMIN1(DATA(3), DATA(4))
         YMAX = AMAX1(DATA(3), DATA(4))
